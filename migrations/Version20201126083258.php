 ...
 */
final class Version20201126083258 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('DROP INDEX IDX_83D44F6FF8697D13');
        $this->addSql('CREATE TEMPORARY TABLE __temp__quack AS SELECT id, author_id, comment_id, content, created_at, picture FROM quack');
        $this->addSql('DROP TABLE quack');
        $this->addSql('CREATE TABLE quack (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, author_id INTEGER NOT NULL, parent_id INTEGER DEFAULT NULL, content CLOB NOT NULL COLLATE BINARY, created_at DATETIME NOT NULL, picture VARCHAR(255) DEFAULT NULL COLLATE BINARY, CONSTRAINT FK_83D44F6FF675F31B FOREIGN KEY (author_id) REFERENCES ducks (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_83D44F6F727ACA70 FOREIGN KEY (parent_id) REFERENCES quack (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO quack (id, author_id, parent_id, content, created_at, picture) SELECT id, author_id, comment_id, content, created_at, picture FROM __temp__quack');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('DROP INDEX IDX_83D44F6FF675F31B');
        $this->addSql('DROP INDEX IDX_83D44F6F727ACA70');
        $this->addSql('CREATE TEMPORARY TABLE __temp__quack AS SELECT id, author_id, parent_id, content, created_at, picture FROM quack');
        $this->addSql('DROP TABLE quack');
        $this->addSql('CREATE TABLE quack (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, author_id INTEGER NOT NULL, content CLOB NOT NULL, created_at DATETIME NOT NULL, picture VARCHAR(255) DEFAULT NULL, comment_id INTEGER DEFAULT NULL, critik VARCHAR(255) DEFAULT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO quack (id, author_id, comment_id, content, created_at, picture) SELECT id, author_id, parent_id, content, created_at, picture FROM __temp__quack');
        $this->addSql('DROP TABLE __temp__quack');
        $this->addSql('CREATE INDEX IDX_83D44F6FF675F31B ON quack (author_id)');
